...
module.exports=async function handler(req,res){
  ...
  const auth=clean(req.headers.authorization);const accessToken=auth.replace(/^Bearer\s+/i,'');
  ...
  try{
    ...
    const user=await readJson(userResponse);if(!user?.id)return json(res,401,{error:'Invalid authentication token'});
    const propertyId=clean(req.body?.property_id||req.body?.propertyId);if(!propertyId)return json(res,400,{error:'property_id is required'});
    ...
    const properties=await readJson(propertyResponse);const property=Array.isArray(properties)?properties[0]:null;if(!property)return json(res,404,{error:'Draft property not found'});
    let isAdmin=false;const profileResponse=await fetch(`${supabaseUrl}/rest/v1/profiles?select=role&id=eq.${encodeURIComponent(user.id)}&limit=1`,{headers:baseHeaders});
    ...
    if(!rawUprn)return json(res,400,{error:'An exact UPRN match is required before EPC verification. Complete property identity verification first.',code:'UPRN_REQUIRED'});
    ...
    const params=new URLSearchParams({uprn,page_size:'100'});
    ...
    if(epcResponse.status===404)return json(res,200,{matched:false,rows:[],message:'No domestic EPC certificate matched the exact UPRN.'});
    ...
    const exactRows=rows.filter(r=>String(r.uprn||'').padStart(12,'0')===uprn);
    if(!exactRows.length)return json(res,200,{matched:false,rows:[],message:'The EPC service returned records, but none matched the exact property UPRN.'});
    const selected=exactRows[0];const rating=clean(selected.currentEnergyEfficiencyBand).toUpperCase();const certificate=clean(selected.certificateNumber);
    if(!rating)return json(res,200,{matched:false,rows:exactRows,warning:'EPC record found for the UPRN but no current energy efficiency band was returned.'});
    const capturedAt=new Date().toISOString();
    const extractedValue=JSON.stringify({rating,certificateNumber:certificate||null,uprn:selected.uprn?String(selected.uprn):uprn,postcode:selected.postcode||property.postcode||null,registrationDate:selected.registrationDate||null,lodgementDate:selected.lodgementDate||null});
    ...
    const evidencePayload={property_id:property.id,field_key:'epc_rating',source_type:'government_api',source_name:EPC_SOURCE,source_url:certificate?EPC_REGISTER_URL:EPC_SOURCE_URL,match_method:'exact_uprn',extracted_value:extractedValue,evidence_status:'verified',evidence_hash:evidenceHash,captured_at:capturedAt,reviewed_at:capturedAt,notes:'Matched against the current domestic EPC developer API using the exact property UPRN. EPC data covers England and Wales.',source_record_id:certificate||null,source_dataset:'Domestic EPC',source_published_at:selected.lodgementDate||selected.registrationDate||null};
    ...
    return json(res,200,{matched:true,property_id:property.id,source:EPC_SOURCE,certificateNumber:certificate||null,rating,matchMethod:'exact_uprn',uprn:selected.uprn?String(selected.uprn):uprn,postcode:selected.postcode||property.postcode||null,evidenceHash});
