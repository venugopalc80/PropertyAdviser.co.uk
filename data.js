window.HAVENLY_PROPERTIES = [
{id:1,dbId:'10000000-0000-4000-8000-000000000001',price:475000,title:'3 bedroom semi-detached house',location:'Bristol, BS7',beds:3,baths:2,area:'1,245 sq ft',epc:'B',tenure:'Freehold',council:'Band D',parking:'Driveway',garden:'Rear garden',score:94,tag:'Featured',status:'Verified',image:'https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?auto=format&fit=crop&w=1600&q=90'},
{id:2,dbId:'10000000-0000-4000-8000-000000000002',price:625000,title:'2 bedroom apartment',location:'London, SE1',beds:2,baths:2,area:'885 sq ft',epc:'C',tenure:'Leasehold',council:'Band F',parking:'Permit',garden:'Shared',score:91,tag:'City living',status:'New',image:'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=1600&q=90'},
{id:3,dbId:'10000000-0000-4000-8000-000000000003',price:385000,title:'4 bedroom detached house',location:'Leeds, LS16',beds:4,baths:2,area:'1,890 sq ft',epc:'C',tenure:'Freehold',council:'Band E',parking:'Garage + drive',garden:'Large rear garden',score:96,tag:'Family home',status:'Verified',image:'https://images.unsplash.com/photo-1605146769289-440113cc3d00?auto=format&fit=crop&w=1600&q=90'},
{id:4,dbId:'10000000-0000-4000-8000-000000000004',price:510000,title:'3 bedroom townhouse',location:'Manchester, M20',beds:3,baths:2,area:'1,430 sq ft',epc:'B',tenure:'Freehold',council:'Band D',parking:'Allocated',garden:'Private garden',score:95,tag:'Garden',status:'Verified',image:'https://images.unsplash.com/photo-1605276374104-dee2a0ed3cd6?auto=format&fit=crop&w=1600&q=90'},
{id:5,dbId:'10000000-0000-4000-8000-000000000005',price:795000,title:'4 bedroom period home',location:'Bath, BA1',beds:4,baths:3,area:'2,240 sq ft',epc:'D',tenure:'Freehold',council:'Band G',parking:'On-street',garden:'Courtyard',score:88,tag:'Period',status:'New',image:'https://images.unsplash.com/photo-1600585154526-990dced4db0d?auto=format&fit=crop&w=1600&q=90'},
...
(function(){
  ...
  async function boot(){
    ...
    if(page==='property.html'){
      ...
      try{
        const id=Number(new URLSearchParams(location.search).get('id'))||1,p=window.HAVENLY_PROPERTIES.find(x=>x.id===id),btn=document.getElementById('saveBtn');
        if(!p||!btn)return;
    ...
    if(page==='enquiry.html'){
      ...
      form.addEventListener('submit',async e=>{
        const id=Number(new URLSearchParams(location.search).get('id'))||1,p=window.HAVENLY_PROPERTIES.find(x=>x.id===id),u=await window.havenlyData.user();
        if(!p||!u||!p.dbId)return;
